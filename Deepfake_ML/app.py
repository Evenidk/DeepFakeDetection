...
def predict():
    ...
    return f'The model predicts this is {result} with {confidence * 100:.2f}% confidence.'
...
def extract_frames(video_path, frame_rate=1):
    ...
    count = 0
    while success:
        if count % frame_rate == 0:  # Extract frame based on the frame rate
            frames.append(frame)
        success, frame = video.read()
        count += 1
    video.release()
...
def predict_frame(frame):
    """Predict whether the frame is real or fake using the model."""
...
def predict_video():
    """Handle video file uploads and predict whether it's real or fake."""
    ...
    # Cast numpy float32 to Python float
